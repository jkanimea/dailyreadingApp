...
public class IngestBibleCommand
{
    ...
    public IngestBibleCommand(string connectionString, bool dryRun)
    {
    ...
    {
        Console.WriteLine("Downloading KJV Bible data...");
        var json = await _http.GetStringAsync(KjvJsonUrl);
        using var doc = JsonDocument.Parse(json);
        var root = doc.RootElement;

        Console.WriteLine($"Processing {root.GetArrayLength()} books...");
        ...
        if (await context.Set<BibleBook>().AnyAsync())
        {
            Console.WriteLine("Bible data already exists. Skipping.");
            return 0;
        }

        int totalVerses = 0;
        var bookEntities = new List<BibleBook>();
        ...
        foreach (var bookEl in root.EnumerateArray())
        {
            ...
            {
                Console.WriteLine($"  Skipping unknown abbreviation: {abbrev}");
                continue;
            }
            ...
            bookEntity.ChapterCount = chapters.GetArrayLength();
            ...
            Console.WriteLine($"  {bookName}: {bookEntity.ChapterCount} chapters, {verses.Count} verses");
        }

        if (!_dryRun)
        {
            await context.SaveChangesAsync();
            Console.WriteLine($"Saved {bookEntities.Count} books, {totalVerses} total verses");
        }

        Console.WriteLine($"Total: {bookEntities.Count} books, {totalVerses} verses");
        return 0;
    }
}
