...
build_android_apk() {
    ...
    # Pre-generate version so both build paths get the right commit count
    local commit_count
    commit_count="$(git -C "$REPO_ROOT" rev-list --count HEAD 2>/dev/null || echo "0")"
    cat > "$REPO_ROOT/frontend/src/environments/version.ts" <<VERSIONEOF
// Auto-generated by deploy.sh — do not edit
export const appVersion = 'v${commit_count}';
VERSIONEOF

    # Patch Facebook OAuth intent filter into AndroidManifest.xml (for both build paths)
    patch_facebook_intent() {
        local manifest="$REPO_ROOT/frontend/android/app/src/main/AndroidManifest.xml"
        [ -f "$manifest" ] || return 0
        if grep -q 'fb1510105297476514' "$manifest" 2>/dev/null; then
            log "Facebook intent filter already present in AndroidManifest.xml"
            return
        fi
        node -e "
const fs = require('fs');
...
console.log('Added Facebook intent filter to AndroidManifest.xml');
"
    }
